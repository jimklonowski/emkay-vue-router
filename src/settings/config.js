//const SYSTEM_BAR_CLASS = 'blue-grey darken-4'
const SYSTEM_BAR_CLASS = 'primary darken-3'

//const APP_BAR_CLASS = 'blue-grey darken-2'
const APP_BAR_CLASS = 'primary darken-1'

//const TOOLBAR_CLASS = 'blue-grey darken-1 white--text'
const TOOLBAR_CLASS = 'primary darken-1 elevation-0'

const COMPONENT_HEADER_CLASS = 'primary darken-1 py-2 white--text'
const SUBHEADER_CLASS = 'overline col-12'
const FOOTER_CLASS = 'primary darken-2 py-2'
//const FIELD_CLASS = 'text-label blue-grey--text text--darken-2'
const FIELD_CLASS = 'display-field'
const LABEL_CLASS = 'display-label'

const config = {
  SYSTEM_BAR_CLASS,
  APP_BAR_CLASS,
  COMPONENT_HEADER_CLASS,
  FOOTER_CLASS,
  TOOLBAR_CLASS,
  SUBHEADER_CLASS,
  FIELD_CLASS,
  LABEL_CLASS
}

export default config
